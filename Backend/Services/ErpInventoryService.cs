...
public class ErpInventoryService : IInventoryService
{
    ...
    public ErpInventoryService(HttpClient http) => _http = http;
    ...
    public async Task IncreaseStockAsync(string materialCode, decimal quantity)
    {
        var response = await _http.PostAsJsonAsync($"materials/{materialCode}/increase-stock", new { quantity });
        response.EnsureSuccessStatusCode();
    }
}